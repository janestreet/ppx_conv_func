...
let raise_unsupported ~loc s =
  Location.raise_errorf ~loc "Unsupported use of %s (you can only use it on records)." s
;;

type simple_processor = Location.t -> field_name:string -> expression
...
module Of_simple (S : Simple) = struct
  ...
  let conversion_of_type =
    conversion_of_type
      ~conversion
      ~conversion_name:S.conversion_name
      ~function_name:S.function_name
      ~merge_recursive:S.merge_recursive
  ;;
end
...
module Of_complete (S : Complete) = struct
  let conversion loc ~field_name ~(id : Longident.t Located.t) =
    ...
    | Lident "sexp_option" | Lident "option" -> S.option loc ~field_name
    | Lident "ref"
    | Ldot (Lident "Lazy", "t")
    | Lident "lazy_t"
    | Ldot (Lident "Hashtbl", "t")
...
module Gen_sig = struct
  ...
  let generate ~extension_name ~nil ~record ~loc ~path:_ (_rf, tds) =
    match tds with
    | [ td ] -> fields_of_ty td ~extension_name ~nil ~record
...
let arg_label_of_string s : Asttypes.arg_label =
  ...
  then Optional (String.drop_prefix s 1)
  else Labelled s
;;
...
module Gen_struct = struct
    ...
    let init =
      [%expr
        [%e fields_module ~record_name ~loc ~suffix:"fold"]
          ~init:[%e if pass_acc then [%expr acc] else [%expr []]]]
    in
    let body = make_body ~lds ~init loc conversion_of_type in
    let anonymous = anonymous loc in
    let func =
      let arguments = if pass_anonymous then [ anonymous; anonymous ] else [] in
  ...
  let generate ~extension_name ~record ~loc ~path:_ (_rf, tds) =
    match tds with
    | [ td ] -> fields_of_ty td ~extension_name ~record
    | _ -> raise_unsupported ~loc extension_name
  ;;
end
